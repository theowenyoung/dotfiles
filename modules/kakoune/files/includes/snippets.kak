...
hook -group snippets global WinSetOption 'snippets=$' %{
...
def -override snippets-expand-trigger -params ..1 %{
    eval -save-regs '/snc' %{
...
hook  -group snippets global WinSetOption 'snippets_auto_expand=false$' %{
    rmhooks window snippets-auto-expand
}
hook  -group snippets global WinSetOption 'snippets_auto_expand=true$' %{
    rmhooks window snippets-auto-expand
...
def -override snippets-impl -hidden -params 1.. %{
    eval %sh{
        use=$1
        shift 1
        index=4
...
def -override snippets -params 1 -shell-script-candidates %{
    eval set -- "$kak_quoted_opt_snippets"
    if [ $(($#%3)) -ne 0 ]; then exit; fi
    while [ $# -ne 0 ]; do
        printf '%s\n' "$1"
...
def -override snippets-menu-impl -hidden -params .. %{
    eval %sh{
        if [ $(($#%3)) -ne 0 ]; then exit; fi
        printf 'menu'
        i=1
...
def -override snippets-menu %{
    snippets-menu-impl %opt{snippets}
}

def -override snippets-info %{
...
def -override snippets-insert -hidden -params 1 %<
    eval -save-regs 's' %<
